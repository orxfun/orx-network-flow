...
fn main() {
    ...
    let solver = McnfSolver::aon_wait_dd(&nw, Default::default(), cplex_solver());
    let stats = solver.stats();
    solver.display_lp();
    solver.export_lp("target/aon_wait_dd_nw.lp").expect("lp");
    let solution = solver.solve().unwrap();

    let dot = dot.with_solution(&solution).with_stats(stats);
    dot.create_svg_file("target/aon_wait_dd_nw.dot", "target/aon_wait_dd_nw.svg")
        .unwrap();
