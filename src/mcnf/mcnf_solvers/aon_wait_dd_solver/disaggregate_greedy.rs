...
pub fn disaggregate_dd_greedy<V: Variant>(
    nw: &AonWaitNw<'_, V>,
    dd: SpaceTime,
    ...
    let mut dd_vertex = None;
    let mut ro_to_vertex: BTreeMap<SpaceTime, VIdx> = BTreeMap::new();
    for (v, vertex) in g.enumerated_vertices() {
        match vertex.data() {
            AonWaitVertex::DueDes(x) if *x == dd => dd_vertex = Some(v),
            AonWaitVertex::ReadyOri(ro) => {
                ro_to_vertex.insert(*ro, v);
            }
            _ => {}
        }
    }
    let Some(dd_vertex) = dd_vertex else {
        return;
    };
    ...
    for &commodity in commodities {
        ...
        let ro = p.commodity_by_idx(commodity).origin();
        if let Some(&v) = ro_to_vertex.get(&ro) {
            originating_by_vertex[v] += flow;
        }
    }
    ...
    for &commodity in commodities {
        ...
        node_sunken_by_commodity[dd_vertex][commodity] += flow;
        node_total_sunken[dd_vertex] += flow;
    }
    ...
    for &commodity in commodities {
        ...
        let ro = p.commodity_by_idx(commodity).origin();
        let Some(&ro_vertex) = ro_to_vertex.get(&ro) else {
            continue;
        };
