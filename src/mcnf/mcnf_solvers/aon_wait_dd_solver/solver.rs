...
use crate::mcnf::mcnf_solvers::aon_wait_dd_solver::capacity::add_capacity_constraints;
use crate::mcnf::mcnf_solvers::aon_wait_dd_solver::flow_balance::add_flow_balance_constraints;
use crate::mcnf::mcnf_solvers::aon_wait_dd_solver::obj::objective;
use crate::mcnf::mcnf_solvers::aon_wait_dd_solver::params::AonWaitDdMcnfParams;
use crate::mcnf::mcnf_solvers::aon_wait_dd_solver::sol::create_solution;
use crate::mcnf::mcnf_solvers::aon_wait_dd_solver::vars::define_vars;
use crate::networks::AonWaitNw;
use crate::utils::math_model::lp_solvers_model_to_problem;
use crate::{Variant, mcnf::mcnf_solvers::aon_wait_dd_solver::vars::DdVars};
use alloc::string::{String, ToString};
use good_lp::{Solver, SolverModel};
use lp_solvers::lp_format::LpProblem;

pub struct AonWaitDdMcnfSolver<'a, V: Variant, S: Solver> {
    nw: &'a AonWaitNw<'a, V>,
...
impl<'a, V: Variant, S: Solver> AonWaitDdMcnfSolver<'a, V, S> {
    pub fn compute_stats(nw: &'a AonWaitNw<'a, V>, _params: AonWaitDdMcnfParams) -> McnfStats {
        ...
        for e in nw.g().edges() {
            match e.data() {
                crate::networks::AonWaitEdge::Bypass(_) => bypass_edges += 1,
        ...
        // One flow-balance per (dd, vertex) + one capacity per transport with at least one tracked edge.
        let mut capacity_constraints = 0usize;
        for (_, edges) in nw.transport_edges() {
            if !edges.is_empty() {
                capacity_constraints += 1;
            }
        }
        let num_constraints = dd_count * nw.g().v() + capacity_constraints;
    ...
    pub fn build(nw: &'a AonWaitNw<'a, V>, params: AonWaitDdMcnfParams, solver: S) -> Self {
