use crate::mcnf::mcnf_solvers::aon_wait_ro_solver::{AonWaitRoMcnfParams, AonWaitRoMcnfSolver};
...
fn precomputed_stats_match_built_model_stats() {
    ...
    let precomputed = AonWaitRoMcnfSolver::<_, LpSolver<Cplex>>::compute_stats(
        &nw,
        AonWaitRoMcnfParams::default(),
    );

    let solver = LpSolver(Cplex::with_command("cplex".to_string()));
    let built = AonWaitRoMcnfSolver::build(&nw, AonWaitRoMcnfParams::default(), solver);
