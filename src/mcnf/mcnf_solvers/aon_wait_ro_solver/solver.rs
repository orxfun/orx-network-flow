...
impl<'a, V: Variant, S: Solver> AonWaitRoMcnfSolver<'a, V, S> {
    pub fn compute_stats(nw: &'a AonWaitNw<'a, V>, _params: AonWaitRoMcnfParams) -> McnfStats {
        ...
        // One flow-balance per (ro, vertex) + one capacity per transport with at least one tracked edge.
        ...
        let num_constraints = ro_count * nw.g().v() + capacity_constraints;
