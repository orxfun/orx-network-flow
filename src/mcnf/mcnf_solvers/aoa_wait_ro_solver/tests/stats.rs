use crate::mcnf::mcnf_solvers::aoa_wait_ro_solver::{AoaWaitRoMcnfParams, AoaWaitRoMcnfSolver};
...
fn precomputed_stats_match_built_model_stats() {
    ...
    let precomputed = AoaWaitRoMcnfSolver::<_, LpSolver<Cplex>>::compute_stats(
        &nw,
        AoaWaitRoMcnfParams::default(),
    );

    let solver = LpSolver(Cplex::with_command("cplex".to_string()));
    let built = AoaWaitRoMcnfSolver::build(&nw, AoaWaitRoMcnfParams::default(), solver);
