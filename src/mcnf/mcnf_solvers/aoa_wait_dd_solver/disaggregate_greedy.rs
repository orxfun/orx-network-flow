...
pub fn disaggregate_dd_greedy<V: Variant>(
    ...
    let mut st_to_vertex: BTreeMap<SpaceTime, VIdx> = BTreeMap::new();
    for (v, vertex) in g.enumerated_vertices() {
        st_to_vertex.insert(vertex.data().0, v);
    }

    let Some(&dd_vertex) = st_to_vertex.get(&dd) else {
        return;
    };
    ...
    for &commodity in commodities {
        ...
        if let Some(&v) = st_to_vertex.get(&ro) {
    ...
    for &commodity in commodities {
        ...
        let ro = p.commodity_by_idx(commodity).origin();
        let Some(&ro_vertex) = st_to_vertex.get(&ro) else {
            continue;
        };
